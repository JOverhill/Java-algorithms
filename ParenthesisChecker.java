    ...
    */
    public static int checkParentheses(StackInterface<Character> stack, String fromString) throws ParenthesesException {
      ...
      for (int index = 0; index < fromString.length(); index++){
            ...
            if(!insideQuotes && (fromString.charAt(index)== '{' || fromString.charAt(index)== '[' || fromString.charAt(index)== '(') ) {
               stack.push(fromString.charAt(index));
            }
            if(!insideQuotes && (fromString.charAt(index)== '}' || fromString.charAt(index)== ']' || fromString.charAt(index)== ')') ) {
               if(stack.size() == 0) {
                  throw new ParenthesesException("Empty stack / no opening parenthesis", lineNumber, columnNumber, -1);
               }
               try { 
                  Character poppedElement = stack.pop();
                  
                  if(fromString.charAt(index) == '}' && poppedElement != '{') {
                     throw new ParenthesesException("Parenthesis didn't match.", lineNumber, columnNumber, -3);
                  }
                  if(fromString.charAt(index) == ']' && poppedElement != '[') {
                     throw new ParenthesesException("Parenthesis didn't match.", lineNumber, columnNumber, -3);
                  }
                  ...
                  if(fromString.charAt(index) == '}' && poppedElement == '{') {
                     parenthesisCount = parenthesisCount + 2;
                  }
                  if(fromString.charAt(index) == ']' && poppedElement == '[') {
                     parenthesisCount = parenthesisCount + 2;
                  }
                  ...
                  throw new ParenthesesException("Popped element was null", lineNumber, columnNumber, -4);
               }
         ...
         if(stack.size() != 0) {
            throw new ParenthesesException("Stack not empty after checking all the characters", lineNumber, columnNumber, -2);
         }
         if(insideQuotes) {
            throw new ParenthesesException("Uneven number of quotes", lineNumber, columnNumber, -4);
      }   
