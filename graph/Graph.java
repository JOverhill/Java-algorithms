...
public class Graph<T> {
   ...
   public void addUnDirectedEdge(Vertex<T> source, Vertex<T> destination, double weight) {
      addDirectedEdge(source, destination, weight);
   ...
   public List<Vertex<T>> breadthFirstSearch(Vertex<T> from, Vertex<T> target) {
      ...
      while(!queue.isEmpty()) {
         ...
         List<Edge<T>> edges = getEdges(vertex);
   ...
   public List<Vertex<T>> depthFirstSearch(Vertex<T> from, Vertex<T> target) {
      ...
      while(!stack.isEmpty()) {
         List<Edge<T>> neighbors = getEdges(stack.peek());
